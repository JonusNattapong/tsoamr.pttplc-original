...
data_autopoll_error = f""" SELECT DISTINCT TAG_ID FROM AMR_ERROR where TRUNC(DATA_DATE) =  TO_DATE('{date_system}', 'DD-MM-YYYY') and REPEAT = 3"""
...
input_dataautopoll = f"""UPDATE AMR_DATA_AUTOPOLL SET  ALL_SUCCESS = '{df_data_autopoll_cleaned.shape[0]}', ALL_ERROR = '{df_data_autopoll_error.shape[0]}', METER_POLL_REPEAT3 = '{df_data_autopoll_cleaned.shape[0]}', ERROR_REPEAT3 = '{df_data_autopoll_error.shape[0]}' WHERE DATA_DATE =  TO_DATE('{date_system}', 'DD-MM-YYYY')"""
