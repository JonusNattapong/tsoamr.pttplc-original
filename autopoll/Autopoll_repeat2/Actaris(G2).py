...
log_file_error = 'C:\\Users\\Administrator\\Desktop\\autopoll\\Autopoll_repeat2\\Actaris(G2).log'
...
query = """
            SELECT
                ...
                FROM
                    ...
                    AMR_PORT_INFO
                WHERE
                    AMR_FIELD_METER.METER_POLL_REPEAT1=1 AND
                            ...
                            if matching_records:
                                ...
                                for row in billing_data_all_dicts:
                                    ...
                                    sql_text_billing_insert = f"""
                                        ...
                                        (METER_ID, METER_STREAM_NO, DATA_DATE, CORRECTED_VOL, UNCORRECTED_VOL, AVR_PF, AVR_TF) 
                                        VALUES 
                                        ('{METERID}', '{run}', TO_DATE('{date_formatted}', 'DD-MM-YYYY'), 
                                        '{row['CORRECTED_VOL']}', '{row['UNCORRECTED_VOL']}', '{formatted_avr_pf}', '{formatted_avr_tf}')
                                    """
                                        ...
                                        if non_matching_records:
                                            ...
                                            for record in non_matching_records:
                                                ...
                                                sql_text_billing_insert = """
                                                    INSERT INTO AMR_BILLING_DATA_ERROR 
                                                    (METER_ID, METER_STREAM_NO, DATA_DATE, CORRECTED_VOL, UNCORRECTED_VOL, AVR_PF, AVR_TF) 
                                                    VALUES (:meter_id, :meter_stream_no, TO_DATE(:data_date, 'DD-MM-YYYY'), :corrected_vol, :uncorrected_vol, :avr_pf, :avr_tf)
                                                """
                                                with cx_Oracle.connect(username, password, f"{hostname}:{port}/{service_name}") as connection:
                                                        with connection.cursor() as cursor:
...
##############################################################
        ...
        update_pm = f"""UPDATE AMR_FIELD_METER SET METER_POLL_REPEAT1 = '0' WHERE METER_ID = '{METERID}' AND METER_STREAM_NO = '{run}'"""
        print("update_pm",update_pm)
        with cx_Oracle.connect(username, password, f"{hostname}:{port}/{service_name}") as connection:
                                        with connection.cursor() as cursor:
        ...
        error_delete = f"DELETE FROM AMR_ERROR WHERE TRUNC(DATA_DATE) = TO_DATE('{date_system}', 'DD-MM-YYYY') AND METER_ID = '{METERID}' AND METER_STREAM_NO = '{run}' AND EVC_TYPE = '{evc_type}'"
        
        with cx_Oracle.connect(username, password, f"{hostname}:{port}/{service_name}") as connection:
                    with connection.cursor() as cursor:
                        cursor.execute(error_delete)  
                    connection.commit()  
                    print("DELETE error_delete successful")
        ...
        GRANT_UPDATE = """GRANT UPDATE ON AMR_FIELD_METER TO PTT_PIVOT"""
        update_pm = f"""UPDATE AMR_FIELD_METER SET METER_POLL_REPEAT2 = '1' WHERE METER_ID = '{METERID}' AND METER_STREAM_NO = '{run}'"""
        update_pm_REPEAT1 = f"""UPDATE AMR_FIELD_METER SET METER_POLL_REPEAT1 = '0' WHERE METER_ID = '{METERID}' AND METER_STREAM_NO = '{run}'"""
        update_error_datetime = f"""UPDATE AMR_ERROR SET DATA_DATE = TO_DATE('{current_time_error}', 'DD-MM-YYYY HH24:MI:SS') WHERE TRUNC(DATA_DATE) = TO_DATE('{date_system}', 'DD-MM-YYYY') AND METER_ID = '{METERID}' AND METER_STREAM_NO = '{run}'"""
        ...
        with cx_Oracle.connect(username, password, f"{hostname}:{port}/{service_name}") as connection:
                                        with connection.cursor() as cursor:
                                            ...
                                            cursor.execute(update_pm)
                                            connection.commit()
                                            cursor.execute(update_pm_REPEAT1)
                                            connection.commit()
                                            cursor.execute(update_error_datetime)
                                            connection.commit()
                                            print("update METER_POLL_REPEAT1 successfully")
...
log_file_info = 'C:\\Users\\Administrator\\Desktop\\autopoll\\Autopoll_repeat2\\total.log'
...
logger_info.info(f"Actaris(G2) : Total sets processed: {total_sets}, Successful rows: {successful_rows}, Errors: {error_count}") 
...
log_file_info_site = 'C:\\Users\\Administrator\\Desktop\\autopoll\\Autopoll_repeat2\\total_site.log'
...
logger_info_site.info(f"Actaris(G2) : Total site processed: {total_site}, Successful rows: {len(counter_1.keys())}, Errors: {len(counter.keys())}") 
