...
TOTAL_AUTO_REPEAT = 3  # auto 3 time
...
if __name__ == '__main__':
    ...
    parser.add_argument("--repeat", type=int, default=1, help="Input  --repeat xx for the round repeat")
    parser.add_argument("--typeid", type=int, default=5, help="Input --typeid xx for EVC ID ")
    
    args = parser.parse_args()
    repeat_round = args.repeat
    type_id_input = args.typeid

    # TGCIR
    if repeat_round == 1:
        # insert record data_autopoll
        grant_update = f"""GRANT UPDATE ON AMR_FIELD_METER TO {os.getenv("DB_USERNAME")}"""
        clear_repeat_sql = f"""UPDATE AMR_FIELD_METER SET 
                    METER_POLL_REPEAT1 = '1',
                    METER_POLL_REPEAT2 = '1',
                    METER_POLL_REPEAT3 = '1'
                    WHERE METER_STREAM_TYPE LIKE '{type_id_input}' AND
                    METER_ID = 'MET0386'"""
        ...
        with cx_Oracle.connect(username, password, f"{hostname}:{port}/{service_name}") as connection:
                                        with connection.cursor() as cursor:
                                            ...
                                            connection.commit()
                                            cursor.execute(clear_repeat_sql)
                                            connection.commit()
    
    # Condition_repeat
    str_repeat_add = ""
    if repeat_round > 1:
        str_repeat_add = f"AND AMR_FIELD_METER.METER_POLL_REPEAT{(repeat_round)} = '1'"""
    ...
    query = f"select VC_NAME from AMR_VC_TYPE where ID = {type_id_input}"
    rows =  fetch_data(query)
    VCtype_name = rows[0][0]

    # สร้าง Handler เพื่อบันทึกข้อความ error ลงในไฟล์
    # log_file_error = f"C:\\Users\\Administrator\\Desktop\\autopoll\\Autopoll_repeat{repeat_round}\\{VCtype_name}.log"""
    log_file_error = os.getenv("LOG_FILE_PATH") + f"""Autopoll_repeat{repeat_round}\\{VCtype_name}.log"""
    ...
    query = f"""SELECT
                    ...
                    WHERE
                        ...
                        amr_vc_type.id like '{type_id_input}' {str_repeat_add}
                        AND AMR_FIELD_ID.METER_ID = 'MET0386'
                    ORDER BY                    
    ...
    for row in rows:
        try:
            ...
            for i in range(0, len(df_pollRange)):
                ...
                response_i = modbusdrv.tx_socket(sock_i, request_message_i, 1.8, 0)
            ...
            set_text = f"""METER_POLL_REPEAT{repeat_round} = '0'"""
            for i in range(repeat_round + 1, TOTAL_AUTO_REPEAT + 1 ):
                set_text += f" , METER_POLL_REPEAT{i} = '0'"
            update_pm = f"""UPDATE AMR_FIELD_METER SET {set_text} WHERE METER_ID = '{METERID}' AND METER_STREAM_NO = '{run}'"""
            ...
            with cx_Oracle.connect(username, password, f"{hostname}:{port}/{service_name}") as connection:
                ...
                connection.commit()     
                print(f"Site :{Sitename}  Run : {run} Completed")
            ...
            # grant_update = f"""GRANT UPDATE ON AMR_FIELD_METER TO {os.getenv("DB_USERNAME")}"""
            # update_pm = f"""UPDATE AMR_FIELD_METER SET METER_POLL_REPEAT{repeat_round} = '1' WHERE METER_ID = '{METERID}' AND METER_STREAM_NO = '{run}'"""
            ...
            sql_text_billing_insert = f"""
                INSERT INTO AMR_ERROR (
                    METER_ID, METER_STREAM_NO, DATA_DATE, TAG_ID, ERROR_DESC, EVC_TYPE, REPEAT
                ) VALUES (
                    '{METERID}', '{run}', TO_DATE('{current_time_error}', 'DD-MM-YYYY HH24:MI:SS'), 
                    '{Sitename}', '{error_desc}', '{VCtype}', {repeat_round}
                ) 
            """
            with cx_Oracle.connect(username, password, f"{hostname}:{port}/{service_name}") as connection:
                with connection.cursor() as cursor:
    ...
    #EK280
    log_file_info = os.getenv("LOG_FILE_PATH") + f"""Autopoll_repeat{repeat_round}\\total.log"""
    #log_file_info = f"C:\\Users\\Administrator\\Desktop\\autopoll\\Autopoll_repeat{repeat_round}\\total.log"""
    ...
    logger_info.info(f"{VCtype_name} : Total sets processed: {total_sets}, Successful rows: {successful_rows}, Errors: {error_count}") 


    logger_info_site = logging.getLogger('info_logger_site')
    logger_info_site.setLevel(logging.INFO)
    log_file_info_site = os.getenv("LOG_FILE_PATH") + f"""Autopoll_repeat{repeat_round}\\total_site.log"""
    ...
    logger_info_site.info(f"{VCtype_name}+ : Total site processed: {total_site}, Successful rows: {len(counter_1.keys())}, Errors: {len(counter.keys())}") 
